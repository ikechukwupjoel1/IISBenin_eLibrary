-- Creates a function that can only be called by a super_admin to update any institution.
...
BEGIN
  ...
  SET 
    name = new_name,
    theme_settings = jsonb_set(
      COALESCE(theme_settings, '{}'::jsonb),
      '{tagline}',
      to_jsonb(new_tagline)
    ) || jsonb_set(
      COALESCE(theme_settings, '{}'::jsonb),
      '{logo_url}',
      to_jsonb(new_logo_url)
    ) || jsonb_set(
      COALESCE(theme_settings, '{}'::jsonb),
      '{favicon_url}',
      to_jsonb(new_favicon_url)
    )
