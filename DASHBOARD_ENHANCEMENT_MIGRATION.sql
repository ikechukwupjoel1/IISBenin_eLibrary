-- =====================================================
-- SUPER ADMIN DASHBOARD ENHANCEMENT MIGRATION
-- =====================================================
-- This migration adds comprehensive tracking and analytics
...
-- =====================================================
-- Tracks all significant activities across the platform

CREATE TABLE IF NOT EXISTS public.admin_activity_feed (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  activity_type TEXT NOT NULL,
  description TEXT NOT NULL,
  entity_type TEXT, -- 'institution', 'user', 'librarian', 'book', etc.
  entity_id UUID,
...
CREATE INDEX idx_activity_feed_created_at ON public.admin_activity_feed(created_at DESC);
CREATE INDEX idx_activity_feed_type ON public.admin_activity_feed(activity_type);
CREATE INDEX idx_activity_feed_entity ON public.admin_activity_feed(entity_type, entity_id);
CREATE INDEX idx_activity_feed_institution ON public.admin_activity_feed(institution_id);
CREATE INDEX idx_activity_feed_actor ON public.admin_activity_feed(actor_id);
...
-- =====================================================
-- Stores historical system metrics for trend analysis

CREATE TABLE IF NOT EXISTS public.system_metrics (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  metric_name TEXT NOT NULL,
  metric_value NUMERIC NOT NULL,
  metric_unit TEXT, -- 'count', 'percentage', 'bytes', 'seconds', etc.
  dimension JSONB DEFAULT '{}'::jsonb, -- Additional dimensions like institution_id, region, etc.
...
-- =====================================================
-- Tracks active user sessions for accurate session counts
...
CREATE INDEX idx_active_sessions_institution ON public.active_sessions(institution_id);
CREATE INDEX idx_active_sessions_active ON public.active_sessions(is_active) WHERE is_active = true;
...
-- =====================================================
-- Tracks storage usage per institution
...
-- Indexes
CREATE INDEX idx_storage_usage_institution ON public.storage_usage(institution_id);
CREATE INDEX idx_storage_usage_total ON public.storage_usage(total_bytes DESC);
CREATE INDEX idx_storage_usage_calculated ON public.storage_usage(last_calculated_at DESC);
...
BEGIN
  -- TODO: Implement actual storage calculation based on your storage system
  -- This is a placeholder that you'll need to customize based on your file storage

  -- For now, insert/update with zero values
...
-- Get dashboard metrics (optimized query)
...
BEGIN
  ...
  SELECT
    ...
    100::NUMERIC; -- TODO: Get from config
END;
$$;
...
-- Trigger to log institution creation
CREATE OR REPLACE FUNCTION trigger_log_institution_creation()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
BEGIN
...
-- Trigger to log institution suspension/reactivation
CREATE OR REPLACE FUNCTION trigger_log_institution_status_change()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
BEGIN
...
-- Trigger to log librarian invitation
CREATE OR REPLACE FUNCTION trigger_log_librarian_invitation()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
DECLARE
...
-- Function to cleanup old metrics (call periodically)
...
-- 3. Update frontend to use new RPC functions
-- 4. Set up cron jobs for cleanup functions
-- =====================================================
