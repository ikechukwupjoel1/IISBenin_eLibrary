...
export function ChatMessaging() {
  ...
  const [targetAudience, setTargetAudience] = useState(profile?.role === 'staff' ? 'students' : 'all');
  ...
  const handlePostAnnouncement = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!announcementMessage.trim() || !profile) return;
  ...
  return (
    <div className="h-[calc(100vh-200px)] flex flex-col sm:flex-row gap-4">
      ...
      <div className="flex-1 bg-white rounded-xl border border-gray-200 flex flex-col">
        {(profile?.role === 'librarian' || profile?.role === 'staff') && (
          <div className="p-2 border-b border-gray-200 flex gap-2">
            <button
              onClick={() => setMessageType('chat')}
              className={`px-3 py-1 rounded-lg text-sm font-medium ${
                messageType === 'chat' ? 'bg-blue-600 text-white' : 'hover:bg-gray-100'
              }`}
            >
              Chat Message
            </button>
            <button
              ...
              className={`px-3 py-1 rounded-lg text-sm font-medium ${
                messageType === 'announcement' ? 'bg-blue-600 text-white' : 'hover:bg-gray-100'
              }`}
            >
              Announcement
            </button>
